...
# Relies on the iPython API and the python_toolkit libraries:
# https://ipython.readthedocs.io/en/stable/api/index.html
# https://python-prompt-toolkit.readthedocs.io/en/master/pages/reference.html
# Note the example this is based on is outdated. The accept_action attribute
# of buffer no longer exists, and as a far as I can tell there was no way to
# force execution of a cell with the original example. There was no condition
# to break enter from simply inserting a newline. If it were possible for all
# terminal emulators to accept shift or ctrl enter as input, I would simply
# map enter to newline insertion and shift-enter to execution, but this
# doesn't seem universally possible. (For an iTerm2 solution potentially
# see https://github.com/prompt-toolkit/python-prompt-toolkit/issues/529)
# So instead I am stripping blank lines from the input sent to iPython
# from within Vim.
...
#                  text = text[: document.cursor_position]
#              status, indent = shell.check_complete(text)
#              # Example I wrote
#              if status == "complete":
#                  buffer.validate_and_handle()
#              else:
